...
type RPCData = Reject | Resolve | Call | Var | Pipe | PCall;
...
* @returns 追踪数组
*/
function parse_stack(stack: string) {
...
export class Env{
     ...
     * 参数opt可以指定一些特定内容
     *  - `pipe` 用于指定是否为pipe函数。
     *      pipe函数传入的参数只有一个，可以通过readable、writeable收发数据
     *      使用call直接调用pipe会抛出错误，所以无需担心参数类型
     *  - `once` 用于指定是否只允许执行一次
     *      对于临时函数，如接收 2次数据 的情况很有用
     *      执行后会清空这个函数，再次调用将报错
     * 
     * @param name 函数位置(如a.b.c.func)
     * @param func 函数
     * @param opt 参数，可以指定
     */
    static provide(name:string,func:((this:RPC,...data:Array<any>) => void),opt:{
        pipe?: boolean,
        once?: boolean
    } = {}){
    ...
    static set(name:string,value:any){
        this._set(name,this.$global,value);
    }
    ...
    protected __accept(str:string){
        try{
            try{
                ...
                return this.$ws && this.$ws.close(2000,'Bad JSON data');
            }
...
/**
 * RPC For Browser
 * 这个类包装了RPC，只需要一个WebSocket地址即可无限重连保证可用性
 */
export class RPCBrowser extends RPC{
